...
import {
  ...
  reportData, saveBrandSettings, saveRecord, syncNow, updateRepairStatus,
...
const RESOURCES = {
  products: {
    ...
    store: 'products',
    search: ['product_name', 'category', 'brand', 'model', 'imei', 'barcode', 'status'],
    columns: ['product_name', 'category', 'brand', 'imei', 'purchase_price', 'sale_price', 'quantity', 'status'],
    rowMap: inventoryRows,
    fields: [
      ['product_name', 'Product Name', 'text', true], ['category', 'Category', 'select', true, ['Mobile Phones', 'Accessories', 'Spare Parts']], ['brand', 'Brand'], ['model', 'Model'],
      ['imei', 'IMEI'], ['barcode', 'Barcode'], ['purchase_price', 'Purchase Price', 'number'], ['sale_price', 'Sale Price', 'number'],
      ['quantity', 'Quantity', 'number'], ['low_stock_threshold', 'Low Stock Warning', 'number'], ['warranty', 'Warranty'], ['supplier_name', 'Supplier'],
    ],
  },
  customers: {
...
function Inventory({ rows, refresh }) {
  ...
  const blank = { category: 'Mobile Phones', quantity: 0, low_stock_threshold: 3, purchase_price: 0, sale_price: 0 };
  ...
  return <div className="stack"><section className="panel"><ModuleHeader title="Inventory Management" query={query} setQuery={setQuery} onAdd={() => setEditing(blank)} onImport={() => importRef.current?.click()} onExport={() => exportCsv('products.csv', filtered)} onPrint={() => printTable('Inventory Management', filtered, columns)} /><input ref={importRef} className="hidden-input" type="file" accept=".csv" onChange={importFile} /><DataTable rows={filtered} columns={columns} onAdd={() => setEditing(blank)} actions={(row) => <><button className="ghost-btn" onClick={() => printTable('Product Detail', [row], columns)}>View</button><button className="ghost-btn" onClick={() => setEditing(row)}><Edit3 size={15} /> Edit</button><button className="ghost-btn" onClick={() => printBarcode(row)}><Printer size={15} /> Barcode</button><button className="danger-btn" onClick={() => setDeleting(row)}><Trash2 size={15} /> Delete</button></>} /></section>{editing && <ModalShell onClose={() => setEditing(null)}><form onSubmit={submit}><div className="modal-header"><h2>{editing.uuid ? 'Edit Product' : 'Add Product'}</h2><button type="button" className="icon-btn" onClick={() => setEditing(null)} title="Close"><X size={17} /></button></div><div className="modal-body"><div className="form-grid"><label>Product Name<input required value={editing.product_name || ''} onChange={(event) => change('product_name', event.target.value)} /></label><label>Category<select value={editing.category || 'Mobile Phones'} onChange={(event) => change('category', event.target.value)}><option>Mobile Phones</option><option>Accessories</option><option>Spare Parts</option></select></label><label>Brand<input value={editing.brand || ''} onChange={(event) => change('brand', event.target.value)} /></label><label>IMEI / Serial<input value={editing.imei || ''} onChange={(event) => change('imei', event.target.value)} /></label><label>Cost Price<input type="number" value={editing.purchase_price || 0} onChange={(event) => change('purchase_price', event.target.value)} /></label><label>Sale Price<input type="number" value={editing.sale_price || 0} onChange={(event) => change('sale_price', event.target.value)} /></label><label>Stock<input type="number" value={editing.quantity || 0} onChange={(event) => change('quantity', event.target.value)} /></label><label>Low Stock Warning<input type="number" value={editing.low_stock_threshold || 3} onChange={(event) => change('low_stock_threshold', event.target.value)} /></label><label className="span-2">Supplier<input value={editing.supplier_name || ''} onChange={(event) => change('supplier_name', event.target.value)} /></label></div></div><div className="modal-footer"><button type="button" className="ghost-btn" onClick={() => setEditing(null)}>Cancel</button><button className="primary-btn">Save</button></div></form></ModalShell>}{deleting && <DeleteDialog row={deleting} store="products" onClose={() => setDeleting(null)} onDelete={(mode) => remove(deleting, mode)} />}</div>;
}
...
function POS2({ data, brand, refresh }) {
  ...
  const products = filterRows(data.products || [], query, ['product_name', 'barcode', 'imei', 'brand', 'model']);
...
function SettingsPanel({ brand, rows, refresh }) {
  ...
  const fields = [
    ['software_name', 'Software Name'], ['shop_name', 'Shop Name'], ['company_name', 'Company Name'], ['owner_name', 'Owner Name'],
...
function printBarcode(product) {
  printHtml('Barcode', `<div class="brand">DSH</div><h2>${product.product_name}</h2><svg id="barcode"></svg><p>${product.barcode || product.imei || product.uuid}</p>`);
}
