...
const MODULE_LABELS = {
  pos: 'Sales POS', products: 'Inventory', customers: 'Customers', purchases: 'Purchases', expenses: 'Expenses',
  reports: 'Reports', sales: 'Sales', patients: 'Patients', hospitalBilling: 'Hospital Billing', labReports: 'Lab Reports',
  radiologyReports: 'Radiology', repairs: 'Repairs', warrantyClaims: 'Warranty Claims', traderRetailers: 'Retailers', traderRecoveries: 'Recovery',
};

const BUSINESS_ACTIONS = {
  hospital: [['patients', 'New Patient', 'Patient registration'], ['hospitalBilling', 'Billing', 'Open hospital billing'], ['labReports', 'Lab Reports', 'Review lab work'], ['radiologyReports', 'Radiology', 'Review radiology'], ['expenses', 'Add Expense', 'Record expense'], ['reports', 'View Reports', 'Hospital insights']],
  mobile_shop: [['pos', 'New Sale', 'Open mobile POS'], ['products', 'Add Device', 'Inventory / IMEI'], ['customers', 'New Customer', 'Customer record'], ['repairs', 'New Repair', 'Repair jobs'], ['warrantyClaims', 'Warranty', 'Claims & service'], ['reports', 'View Reports', 'Business insights']],
  pharmacy: [['pos', 'New Sale', 'Open pharmacy POS'], ['products', 'Add Medicine', 'Inventory / batch'], ['customers', 'New Customer', 'Patient / customer'], ['purchases', 'New Purchase', 'Stock in'], ['expenses', 'Add Expense', 'Record expense'], ['reports', 'View Reports', 'Pharmacy insights']],
  traders: [['pos', 'New Sale', 'Create trade sale'], ['products', 'Add Stock', 'Inventory'], ['traderRetailers', 'New Retailer', 'Retailer management'], ['traderRecoveries', 'Recovery', 'Collect payment'], ['purchases', 'New Purchase', 'Stock in'], ['reports', 'View Reports', 'Distribution insights']],
  default: [['pos', 'New Sale', 'Open POS'], ['products', 'Add Product', 'Inventory'], ['customers', 'New Customer', 'Customer record'], ['purchases', 'New Purchase', 'Stock in'], ['expenses', 'Add Expense', 'Record expense'], ['reports', 'View Reports', 'Business insights']],
};
...
function today(value) { return String(value || '').slice(0, 10) === new Date().toISOString().slice(0, 10); }
function money(value) { return new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR', maximumFractionDigits: 0 }).format(Number(value || 0)); }
...
async function loadDashboard() {
  const [brand, sales, products, customers, purchases, expenses, patients, bills, labReports, radiologyReports, repairs, warrantyClaims, retailers, recoveries] = await Promise.all([
    getBrandSettings(), listRecords('sales'), listRecords('products'), listRecords('customers'), listRecords('purchases'), listRecords('expenses'),
    listRecords('patients'), listRecords('hospital_bills'), listRecords('lab_reports'), listRecords('radiology_reports'), listRecords('repairs'),
    listRecords('warranty_claims'), listRecords('trader_retailers'), listRecords('trader_recoveries'),
  ]);
  ...
  const receivables = sales.reduce((sum, row) => sum + Number(row.balance || 0), 0) + customers.reduce((sum, row) => sum + Number(row.balance || 0), 0) + bills.reduce((sum, row) => sum + Number(row.balance || 0), 0);
  ...
  if (receivables > 0) alerts.push({ id: 'receivables', title: `${money(receivables)} receivables`, description: 'Review outstanding balances.', action: key === 'hospital' ? 'hospitalBilling' : 'sales', tone: 'warning' });
  if (key === 'pharmacy') {
  ...
  if (key === 'hospital') metrics = [
  ...
  else if (key === 'mobile_shop') metrics = [
    { key: 'sales', label: 'Today Sales', value: money(salesValue), tone: 'primary' }, { key: 'orders', label: 'Orders', value: salesToday.length, tone: 'success' },
    { key: 'receivables', label: 'Receivables', value: money(receivables), tone: 'warning' }, { key: 'stock', label: 'Open Repairs', value: repairs.filter((row) => !['Completed', 'Delivered', 'Cancelled'].includes(row.status)).length, tone: 'danger' },
  ];
    ...
    metrics = [
      { key: 'sales', label: 'Today Sales', value: money(salesValue), tone: 'primary' }, { key: 'orders', label: 'Orders', value: salesToday.length, tone: 'success' },
      { key: 'receivables', label: 'Receivables', value: money(receivables), tone: 'warning' }, { key: 'stock', label: 'Near Expiry', value: nearExpiry.length, tone: 'danger' },
    ];
  } else if (key === 'traders') metrics = [
    { key: 'sales', label: 'Today Sales', value: money(salesValue), tone: 'primary' }, { key: 'orders', label: 'Retailers', value: retailers.length, tone: 'success' },
    { key: 'receivables', label: 'Outstanding', value: money(receivables), tone: 'warning' }, { key: 'stock', label: 'Today Recovery', value: money(recoveries.filter((row) => today(row.date || row.created_at)).reduce((sum, row) => sum + Number(row.amount || 0), 0)), tone: 'danger' },
  ];
  else metrics = [
    { key: 'sales', label: 'Today Sales', value: money(salesValue), tone: 'primary', subtext: `${salesToday.length} invoices` }, { key: 'orders', label: 'Orders', value: salesToday.length, tone: 'success' },
    { key: 'receivables', label: 'Receivables', value: money(receivables), tone: 'warning' }, { key: 'stock', label: 'Low Stock', value: lowStock.length, tone: 'danger' },
  ];
  const recentSales = sales.slice(0, 8).map((row) => ({ id: row.uuid, invoice: row.invoice_number, customer: row.customer_name, time: row.sold_at || row.created_at, total: money(row.total), status: row.status || 'Completed' }));
  return { brand, key, metrics, recentSales, alerts };
}
...
function goModule(moduleId) { findModuleButton(moduleId)?.click(); }
...
function mountWhenDashboardIsVisible() {
  ...
  if (!stack || (mountedNode && mountedNode.isConnected)) return;
...
const observer = new MutationObserver(() => {
  if (!mountedNode || !mountedNode.isConnected) { mountedNode = null; root = null; mountWhenDashboardIsVisible(); }
});
function boot() {
