...
import {
  ...
  cleanupStartupData, deleteRecord, deleteRemoteRecord, deleteRemoteRecordsBulk, downloadPdf, ensureDeviceId, exportBackupFile, exportCsv, generateLicense,
...
const MOBILE_SHOP_MODULES = new Set(['dashboard', 'pos', 'sales', 'products', 'customers', 'credit', 'mobileWallets', 'repairs', 'repairReceipts', 'purchases', 'suppliers', 'expenses', 'notifications', 'reports', 'backup', 'users', 'warrantyClaims', 'returns']);
...
const GENERIC_SHOP_MODULES = new Set(['dashboard', 'pos', 'sales', 'products', 'customers', 'credit', 'mobileWallets', 'purchases', 'suppliers', 'expenses', 'notifications', 'reports', 'catalog', 'backup', 'users']);
const PHARMACY_MODULES = new Set(['dashboard', 'pos', 'sales', 'products', 'customers', 'credit', 'mobileWallets', 'purchases', 'suppliers', 'expenses', 'notifications', 'reports', 'catalog', 'medicines', 'backup', 'users']);
const TRADERS_MODULES = new Set(['dashboard', 'pos', 'sales', 'products', 'traderCompanies', 'traderBrands', 'traderTerritories', 'traderRoutes', 'traderSalesmen', 'traderRetailers', 'traderChallans', 'traderRecoveries', 'traderSalesmanLedger', 'traderDistributorLedger', 'purchases', 'suppliers', 'expenses', 'notifications', 'reports', 'catalog', 'backup', 'users']);
...
const BUSINESS_TYPE_RULES = {
  ...
  general_store: {
    ...
    showPackaging: true,
    showTrader: false,
    inventoryColumns: ['product_name', 'category', 'brand', 'barcode', 'pack_size', 'unit', 'quantity', 'packaging_view', 'purchase_price', 'sale_price', 'low_stock_threshold', 'status'],
  ...
  traders: {
    ...
    showPackaging: true,
    showTrader: true,
    inventoryColumns: ['product_name', 'category', 'brand', 'barcode', 'pack_size', 'unit', 'quantity', 'packaging_view', 'purchase_price', 'sale_price', 'status'],
    inventorySearch: ['product_name', 'brand', 'category', 'barcode', 'secondary_barcode', 'qr_code', 'box_barcode', 'carton_barcode', 'sku', 'product_code', 'pack_size', 'unit'],
    posSearch: ['product_name', 'brand', 'category', 'barcode', 'secondary_barcode', 'qr_code', 'box_barcode', 'carton_barcode', 'sku', 'product_code', 'pack_size', 'unit'],
    posCartColumns: ['product_name', 'selected_unit', 'unit_conversion', 'available_stock', 'price', 'quantity', 'line_total'],
    detailFields: ['barcode', 'secondary_barcode', 'qr_code', 'product_code', 'box_barcode', 'carton_barcode', 'sku', 'variant_type', 'pack_size', 'supplier_name'],
...
const RESOURCES = {
  products: {
    ...
    columns: ['product_name', 'category', 'brand', 'imei_numbers', 'unit', 'quantity', 'packaging_view', 'purchase_price', 'total_cost', 'sale_price', 'low_stock_threshold', 'status'],
    ...
    fields: [
      ...
      ['sku', 'SKU'], ['product_code', 'Product Code'], ['barcode', 'Barcode'], ['secondary_barcode', 'Secondary Barcode'], ['qr_code', 'QR Code'], ['box_barcode', 'Box Barcode'], ['carton_barcode', 'Carton Barcode'], ['unit', 'Base Unit', 'select', true, RETAIL_UNITS], ['variant_type', 'Variant Type', 'select', false, RETAIL_UNITS], ['pack_size', 'Pack Size'], ['batch_number', 'Batch Number'],
...
function unitCountLabel(count, unit = 'Piece') {
  const number = Number(count || 0);
  return `${number} ${number === 1 ? singularUnit(unit) : pluralUnit(unit)}`;
}
...
function productBaseUnit(product = {}) {
  return product.unit && !['Single Unit', 'Unit'].includes(product.unit) ? product.unit : 'Piece';
}
...
function stockPackagingBreakdown(product = {}) {
  ...
  for (const unit of units) {
    ...
    if (count > 0) {
      largestParts.push(unitCountLabel(count, unit.unit || unit.label));
      remaining -= count * factor;
    }
  }
  const exactEquivalents = units.map((unit) => {
    const factor = Math.max(1, Number(unit.factor || 1));
    if (factor <= 1 || total < factor || total % factor !== 0) return null;
    return unitCountLabel(Math.floor(total / factor), unit.unit || unit.label);
  }).filter(Boolean);
  return { base: total, text: largestParts.concat(remaining > 0 ? [`${remaining} ${pluralUnit(baseUnit)}`] : []).join(' '), equivalents: exactEquivalents.join(' = ') };
}

function stockPackagingView(product = {}) {
  const breakdown = stockPackagingBreakdown(product);
  const baseUnit = productBaseUnit(product);
  const baseText = `${breakdown.base} ${pluralUnit(baseUnit)}`;
  const parts = [baseText];
  if (breakdown.equivalents) parts.push(breakdown.equivalents);
  else if (breakdown.text && breakdown.text !== baseText) parts.push(breakdown.text);
  return parts.join(' = ');
}
...
function CatalogModule({ rows, products, brand, refresh }) {
  ...
  async function submit(record) {
    ...
    const defaultCost = Math.max(0, Number(record.default_cost ?? record.cost_price ?? record.purchase_price ?? 0));
    const defaultPrice = Math.max(0, Number(record.default_price ?? record.selling_price ?? record.sale_price ?? 0));
    const saved = await saveRecord('master_catalogs', { ...record, name: productName, product_name: productName, business_type: brand?.business_type || 'General Store', default_cost: defaultCost, default_price: defaultPrice });
  ...
  async function removeSelected(mode) {
    const selectedRows = filtered.filter((row) => selectedIds.includes(row.uuid));
    await bulkDeleteEverywhere('master_catalogs', selectedRows, mode);
    setSelectedIds([]);
    setBulkDeleting(false);
    await refresh();
    notify(`${selectedRows.length} catalog items deleted successfully`);
  }
...
function downloadInventoryExcelTemplate(brand = {}) {
  const headers = [
    'product_name', 'category', 'brand', 'sku', 'product_code',
    'barcode', 'secondary_barcode', 'qr_code', 'box_barcode', 'carton_barcode',
    'unit', 'pack_size', 'quantity', 'low_stock_threshold',
    'purchase_price', 'sale_price', 'supplier_name',
    'generic_name', 'strength', 'dosage_form', 'batch_number', 'expiry_date',
    'manufacturer', 'model', 'serial_number', 'imei_numbers', 'warranty',
  ];
  ...
  const rows = [
    headers,
    [
      '', businessType === 'Pharmacy' ? 'Medicine' : (categoriesForBusiness(brand)[0] || ''),
      '', '', '', '', '', '', '', '', 'Piece', '', '0', '3',
      '0', '0', '', '', '', '', '', '', '', '', '', '', '',
    ],
  ];
  const content = [
    `# MSM Inventory Excel Template - ${businessType}`,
    `# Fill the second row or add new rows. Delete these instruction rows before importing if your Excel editor keeps them.`,
    rows[0].map(csvEscape).join(','),
    rows[1].map(csvEscape).join(','),
  ].join('\\r\\n');
  const blob = new Blob([content], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement('a');
  anchor.href = url;
  anchor.download = `MSM-Inventory-Template-${businessType.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.csv`;
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
  notify('Inventory Excel-compatible template downloaded');
}
...
function Inventory({ rows, brand, refresh }) {
  ...
  async function removeSelected(mode) {
    const selectedRows = filtered.filter((row) => selectedIds.includes(row.uuid));
    await bulkDeleteEverywhere('products', selectedRows, mode);
    broadcastInventoryRefresh({ action: 'products-bulk-deleted', product_uuids: selectedRows.map((row) => row.uuid), mode });
    setSelectedIds([]);
    setBulkDeleting(false);
    await refresh();
    notify(`${selectedRows.length} products deleted successfully`);
  }
...
function calculatedProduct(record, finalize = false) {
  ...
  const isEmptyUnits = rawUnitsPerPackage === '' || rawUnitsPerPackage === null || rawUnitsPerPackage === undefined;
  const shouldDefaultUnits = isEmptyUnits && finalize && !record.uuid;
  const unitsPerPackage = isEmptyUnits ? (shouldDefaultUnits ? 1 : '') : safeNumber(rawUnitsPerPackage, 0);
  const unitsPerPackageForMath = unitsPerPackage === '' ? 0 : unitsPerPackage;
  ...
  const legacyQuantity = packageQuantity * unitsPerPackageForMath + looseQuantity;
  const quantity = safeNumber(record.quantity ?? record.stock_quantity, legacyQuantity);
  return {
    ...record,
...
function PackagingPreview({ baseLabel, rows, legacyUnits, stock }) {
  const fallbackUnits = safeNumber(legacyUnits, 0);
  const preview = rows.length ? rows : fallbackUnits > 1 ? [{ key: 'pack', unit: 'Pack', factor: fallbackUnits }] : [];
  const stockQty = Math.max(0, Math.floor(safeNumber(stock, 0)));
  const stockPreview = preview
    .map((row) => {
      const factor = Math.max(1, Number(row.factor || 1));
      if (stockQty < factor || stockQty % factor !== 0) return null;
      return unitCountLabel(Math.floor(stockQty / factor), row.unit || row.label);
    })
    .filter(Boolean);
  return <div className="dashboard-empty"><Boxes size={24} /><strong>Packaging Preview</strong><p>Base Unit: {baseLabel || 'Piece'}</p>{stockQty > 0 && <p>{stockQty} {pluralUnit(baseLabel || 'Piece')}{stockPreview.length ? ` = ${stockPreview.join(' = ')}` : ''}</p>}{preview.length ? preview.map((row) => <p key={row.key}>{row.unit || row.label}: {Math.max(1, Number(row.factor || 1))} {pluralUnit(baseLabel || 'Piece')}</p>) : <p>Add packaging levels to preview Pack, Box and Carton conversion.</p>}</div>;
}
...
function BulkDeleteDialog({ count, label = 'products', onClose, onDelete }) {
  return <ModalShell onClose={onClose} size="small"><div className="modal-header"><h2>Bulk Delete</h2><button type="button" className="icon-btn" onClick={onClose} title="Close"><X size={17} /></button></div><div className="modal-body"><p className="muted">Are you sure you want to delete <strong>{count}</strong> selected {label}?</p><p className="muted">Soft delete hides records and keeps audit history. Permanent delete is admin-only.</p></div><div className="modal-footer"><button className="ghost-btn" onClick={onClose}>Cancel</button><div className="button-row"><button className="danger-btn" onClick={() => onDelete('soft')}><Trash2 size={16} /> Soft Delete</button>{ADMIN_USER && <button className="danger-btn solid" onClick={() => onDelete('permanent')}><Trash2 size={16} /> Permanent Delete</button>}</div></div></ModalShell>;
}
...
async function bulkDeleteEverywhere(store, rows, mode) {
  const selectedRows = (rows || []).filter((row) => row?.uuid);
  if (!selectedRows.length) return { deleted_count: 0 };
  const ids = selectedRows.map((row) => row.uuid);
  for (const row of selectedRows) {
    await deleteRecord(store, row.uuid, mode);
  }
  window.dispatchEvent(new CustomEvent('msm:record-deleted', { detail: { store, uuids: ids, mode, bulk: true } }));
  if (navigator.onLine && localStorage.getItem('dsh_token')) {
    try {
      await deleteRemoteRecordsBulk(store, ids, mode);
      await syncNow();
      window.dispatchEvent(new CustomEvent('msm:record-delete-synced', { detail: { store, uuids: ids, mode, bulk: true } }));
    } catch (error) {
      console.warn(error.message || 'Remote bulk delete failed. It will retry later.');
      notify(error.message || 'Remote bulk delete failed. It will retry later.');
      throw error;
    }
  }
  return { deleted_count: selectedRows.length, deleted_ids: ids };
}
...
function DataTable({ rows, columns, actions, editable, onAdd, selectable = false, selectedIds = [], onSelectionChange }) {
  ...
  const pageIds = pageRows.map((row) => row.uuid).filter(Boolean);
  const allPageSelected = Boolean(pageIds.length) && pageIds.every((uuid) => selectedSet.has(uuid));
  ...
  function toggleRow(row) {
    ...
    next.has(row.uuid) ? next.delete(row.uuid) : next.add(row.uuid);
    onSelectionChange?.([...next]);
  }
  function togglePage() {
    const next = new Set(selectedSet);
    if (allPageSelected) pageIds.forEach((uuid) => next.delete(uuid));
    else pageIds.forEach((uuid) => next.add(uuid));
    onSelectionChange?.([...next]);
  }
  return <div className="data-table"><div className="table-wrap"><table><thead><tr>{selectable && <th className="select-cell"><input type="checkbox" checked={allPageSelected} onChange={togglePage} aria-label="Select all rows on this page" /></th>}{columns.map((column) => <th key={column}><button className="sort-head" onClick={() => toggleSort(column)}>{headerLabel(column)}<span>{sort.key === column ? (sort.dir === 'asc' ? '^' : 'v') : '-'}</span></button></th>)}{actions && <th className="actions-head">Actions</th>}</tr></thead><tbody>{pageRows.length ? pageRows.map((row, index) => <tr key={row.uuid || index}>{selectable && <td className="select-cell"><input type="checkbox" checked={row.uuid ? selectedSet.has(row.uuid) : false} onChange={() => toggleRow(row)} aria-label={`Select ${productDisplayName(row)}`} /></td>}{columns.map((column) => <td key={column} title={String(displayCellValue(row, column) ?? '')}>{editable && ['quantity', 'price'].includes(column) ? <input className="cell-input" type="number" value={row[column]} onChange={(e) => editable(row, column, e.target.value)} /> : editable && column === 'selected_unit' && row.packaging_options?.length ? <select className="cell-input" value={row.selected_unit_key || row.packaging_options[0].key} onChange={(e) => editable(row, column, e.target.value)}>{row.packaging_options.map((unit) => <option key={unit.key} value={unit.key}>{unit.label || unit.unit}</option>)}</select> : format(row[column], column, row)}</td>)}{actions && <td className="actions-cell"><div className="row-actions">{actions(row)}</div></td>}</tr>) : <tr><td colSpan={columns.length + (actions ? 1 : 0) + (selectable ? 1 : 0)}><EmptyRows onAdd={onAdd} /></td></tr>}</tbody></table></div><div className="table-footer"><span>Showing {start}-{end} of {total} records</span><div className="pagination"><button className="ghost-btn" disabled={safePage <= 1} onClick={() => setPage(safePage - 1)}>Previous</button><span>Page {safePage} / {pages}</span><button className="ghost-btn" disabled={safePage >= pages} onClick={() => setPage(safePage + 1)}>Next</button></div></div></div>;
}
...
function format(value, key, row = {}) {
  ...
  if (['default_cost', 'default_price', 'purchase_price', 'sale_price', 'selling_price', 'cost_price', 'unit_cost_price', 'unit_sale_price', 'package_cost_price', 'total_cost', 'amount', 'fee', 'net_amount', 'salary', 'charges', 'repair_charges', 'advance_payment', 'remaining_amount', 'registration_fee', 'doctor_fee', 'medicine_charges', 'injection_charges', 'lab_charges', 'radiology_charges', 'procedure_charges', 'grand_total', 'subtotal', 'discount', 'tax', 'total', 'paid', 'balance', 'profit', 'debit', 'credit', 'total_spent', 'available', 'cash_in', 'sent', 'pending', 'fee_profit', 'mrp'].includes(key)) return money(value);
