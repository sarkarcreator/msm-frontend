...
function PackagingBuilder({ product, onChange }) {
  ...
  const rows = parsePackagingUnits(product?.packaging_units).map((row, index) => ({
    key: row.key || String(row.unit || row.label || `level_${index + 1}`).toLowerCase().replaceAll(' ', '_'),
    unit: row.unit || row.label || '',
    parent_key: row.parent_key || 'base',
    conversion_quantity: row.conversion_quantity || row.contains || 1,
    barcode: row.barcode || '',
    purchase_price: row.purchase_price || row.cost_price || '',
  ...
  const saveRows = (nextRows) => onChange('packaging_units', JSON.stringify(nextRows.map((row) => ({
    ...
    conversion_quantity: Math.max(1, Number(row.conversion_quantity || 1)),
  }))));
  ...
  return <div className="packaging-builder"><div className="module-head compact-head"><h3>Packaging Builder</h3><button type="button" className="ghost-btn" onClick={() => saveRows([...rows, { key: `level_${rows.length + 1}`, unit: '', parent_key: rows[rows.length - 1]?.key || 'base', conversion_quantity: 1, barcode: '', purchase_price: '', sale_price: '' }])}><Plus size={14} /> Add Unit</button></div><div className="data-table compact-table"><div className="table-wrap"><table><thead><tr><th>Unit</th><th>Contains</th><th>Parent</th><th>Barcode</th><th>Purchase Price</th><th>Sale Price</th><th>Stock Factor</th><th></th></tr></thead><tbody>{rows.length ? rows.map((row, index) => {
    const preview = hierarchyPackagingUnits(rows, { unit: baseLabel }).find((item) => item.key === row.key);
